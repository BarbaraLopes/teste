...
INSERT INTO user (username, email, password, activated)
SELECT * FROM (SELECT 'user', 'user@user.com', '$2a$10$r0RFDmpneBVryx.ihHK9gu6FFJQi4nTxQUqzdSTvrPpaKZMxigqpy', true) AS tmp
WHERE NOT EXISTS (
...
INSERT INTO user_authority (username, authority)
SELECT * FROM (SELECT 'user', 'ROLE_USER') AS tmp
WHERE NOT EXISTS (
    SELECT username, authority FROM user_authority WHERE username = 'user' and authority = 'ROLE_USER'
) LIMIT 1;